...
(module smodpath ()
  ...
  (define (anonymous-arg operand options operands)
    (let ((operand (if (not relative-flag) 
		       operand 
  ...
  (define +command-line-options+
    (list (args:make-option (a after) #:required "add next argument to path after ARG"
	  ...
	  (args:make-option (empty) #:none "Add an empty element to the path"
			    (add-empty))
	  ...
	  (args:make-option (exists) #:none "Add next item only if it exists"
			    (set! exists-flag #t))
          (args:make-option (I ignore) #:none "Ignore warnings and do not output them"
                            (set! ignore-flag #t))
