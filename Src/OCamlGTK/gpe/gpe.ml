...
let ep (format : ('b, unit, string, 'a) format4) = Printf.kprintf dbg format
...
class path_edit ?width ?height ?packing ?show () =
  ...
  object (self)
    ...
    method clear_all () =
      ...
      self#do_out_clear ();
      self#do_sw_clear ()
    method go_up (model : GTree.list_store) (view : GTree.view) event =
	...
	selected_rows;
	  ...
	  print_row_path "scroll path" row;
          view#scroll_to_cell (* ~align:(1.0,0.0) *) row#path element_vcol
    method go_down (model : GTree.list_store) (view : GTree.view) event =
      let selected_rows =
	...
	selected_rows;
	  ...
	  print_row_path "scroll path" row;
	    ...
	    element_vcol
    method make_menu (view : GTree.view) (model : GTree.list_store) event =
	...
	let menu = GMenu.menu () in
	ignore (menu#event#connect#unmap 
                  ~callback:(fun e -> menu#destroy (); true));
	let item = GMenu.menu_item ~label:"New" ~packing:menu#add () in
	ignore (item#connect#activate ~callback:(self#add_element ~edit:true));
	...
	| Some (path, column, cell_x, cell_y) ->
	    ...
	    ignore (item#connect#activate ~callback:(self#delete_element iter));
	    List.iter
	      (fun (label, op) ->
		...
		ignore (delete_item#connect#activate ~callback:
		          (fun () ->
		            List.iter
 ...
 *)
	 ...
	 GtkPack.SizeGroup.add_widget clear_group sw_new#as_widget;
      ...
      ignore (new_elem#connect#activate
                ~callback:(self#add_element ~edit:true));
      ignore (sw_clear#connect#clicked ~callback:self#do_sw_clear);
      ignore (sw_new#connect#clicked ~callback:(self#add_element ~edit:true));
      ignore (sw_unique#connect#clicked ~callback:self#unique_elements);
      ignore (in_clear#connect#clicked ~callback:self#do_in_clear);
      ignore (out_clear#connect#clicked ~callback:self#do_out_clear);
      ignore (split_btn#connect#clicked ~callback:self#split);
      ignore (join_btn#connect#clicked ~callback:self#join);
      ignore (print_btn#connect#clicked ~callback:self#print);
      ignore (clear_btn#connect#clicked ~callback:self#clear_all);
      ignore (quit_btn#connect#clicked ~callback:GMain.Main.quit);
      (* Should the file_menu and submenu stuff be in the lets before the
	 object ???  *)
      ignore (file_menu#add_item "Quit" ~key:_Q ~callback:GMain.Main.quit);
      ...
      ignore (path_item#connect#activate ~callback:self#set_in_path);
      let var_item = (GMenu.menu_item ~label:"Variable"
			~packing:get_menu#add ()) in
      ignore (var_item#connect#activate ~callback:self#set_in_path_from_var);
      let user_item = (GMenu.menu_item ~label:"User"
			 ~packing:get_menu#add ()) in
      ignore (user_item#connect#activate ~callback:self#set_in_path_from_user);
      ...
      ignore (out_item#connect#activate ~callback:self#print);
	...
	(Ops.get_ops ());
      ...
      ignore (help_menu#add_item "About" ~key:_A ~callback:about);
	 ...
	 (* view#set_rules_hint true; *) *) 
      ...
      ignore (sw_up#connect#clicked ~callback:(self#go_up store view));
      ignore (sw_down#connect#clicked ~callback:(self#go_down store view));
      ignore (sw_delete#connect#clicked
                ~callback:self#delete_selected_elements);
      (* ??? Cleanup.add_main_ops ops_menu#menu
	 (self :> Cleanup_type.path_edit_type); *)
      ignore (view#event#connect#button_press
                ~callback:(self#make_menu view store))
  end
...
let main () =
  ...
  let gpe_box = GdkPixbuf.from_xpm_data Gpe_icons.gpe_xpm in
  main_window#set_icon (Some (gpe_box));
  main_window#show ();
  GMain.Main.main ()
