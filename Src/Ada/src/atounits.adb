...
procedure Atounits is
   ...
   function Abbreviation (M: Multiplier) return Unbounded_String is
     (if Use_SI then Multipliers (M).SI_Abbreviation else Multipliers (M).BI_Abbreviation);
   ...
   exception
      when CONSTRAINT_ERROR =>
         ...
         exception
            when CONSTRAINT_ERROR =>
               Put_Line ("The value """ & S & """ is not a valid floating point number."); 
...
begin
   loop 
      declare
         Ch: Character := Getopt ("b s K M G T P E Z Y R Q k m g t p e z y r q");
      begin
         case Ch is 
            when ASCII.NUL => exit;
            ...
            when others => 
               Put_Line (Standard_Error, "unknown option -" & Ch);
         end case;
      end;
   end loop;
